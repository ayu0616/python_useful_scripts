...
class MyList(list[T]):
    ...
    def find(self, func: Callable[[T], bool]):
        found_value = self.filter(func)[0]
        if found_value:
            return found_value
        else:
            raise Exception("条件に該当する要素は含まれていません")
    ...
    @classmethod
    def fill(cls, value: T, count: int):
        """
        valueをcount回埋めたリストを返す関数\n
        もとのリストは消えてしまうので注意
        """
        new_list: MyList[T] = cls(value for _ in range(count))
...
ALPHABETS = list("zqawxsecdrfvtbgyhnujmikolp")


def rand_alpha(n: int):
    """n文字のアルファベットをランダムに並べた文字列を出力"""
    s = ""
    for _ in range(n):
